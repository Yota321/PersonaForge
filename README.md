...
Open the file, find QUESTION_BANK near the top of the first <script>
block, and add more question objects to any cluster (or add new
clusters).
Raise TOTAL_QUESTIONS (currently 35) and, if you add clusters,
BASELINE_PER_CLUSTER in the engine section.
Nothing else changes — QuizSession._pickAdaptive() already re-ranks
...
Quiz progress persistence + home icon
The back arrow next to "PersonaForge" is now a home icon.
Clicking it mid-quiz no longer throws answers away. It saves your exact
progress (every answer, the running trait totals, and the adaptive
question plan already built) to localStorage, then takes you to the
landing page, which now shows a "Case in Progress" card: "You answered
14 of 35. Pick up on question 15." with Continue and Start Over options.
Continue restores the session exactly, next question is the very next
unanswered one, all prior answers stay intact and can still be edited
with Back like normal.
Progress also auto-saves after every single answer (not just on the
home click) as a safety net for an accidental tab close, and clears
itself automatically once a quiz actually finishes.
Tested in a real browser: answered 14, went home, confirmed the saved
count and the resume card, continued, confirmed it landed on question
15, then stepped Back through all 14 to confirm none were lost.
v4 update
Additive again, nothing removed.
Adaptive recalculation on edit: changing an earlier answer now
properly invalidates the not-yet-answered part of the question plan, so
the engine picks fresh, updated questions instead of continuing on stale
logic. Verified it never loses already-given answers and always
completes correctly.
Answer Consistency: quietly checks pairs of separate questions that
probe similar ground and reports how consistently you answered, shown
after results. Rebuilt once already during testing after the first
version almost never had enough overlapping questions to say anything;
it's now built on question pairs guaranteed to be asked every run.
Closest MBTI / DISC / Enneagram / Big Five, clearly secondary,
your PersonaForge archetype stays the primary result.
Onboarding: optional Date of Birth, Occupation, and Country, tucked
behind a collapsed toggle, shown only as a light line on your result
card, never used to affect scoring.
PF1 to PF2 upgrade path: viewing an old code now offers a real
9-question mini-quiz (built from existing content, nothing new authored)
that fills in the 5 dimensions PF1 never asked about, then re-encodes a
full PF2 code.
Duo Title and Duo Crest on the compare page (e.g. "Fire & Ice",
"Twin Flames"), plus Fantasy Weapon/Companion/Kingdom and more fun
extras (flower, planet, constellation, gemstone, weather, coffee order).
Micro-interactions: confetti on first reveal, count-up numbers,
card hover glow, and sections that fade in as you scroll to them instead
of all appearing at once. Caught and fixed a real bug here during
testing: the scroll-based reveal left sections invisible in the PDF
export, since printing never fires scroll events. Fixed with a
print-specific override, confirmed opacity reaches 1 under emulated
print media.
A loading animation for compatibility checks: comparing two codes
now shows a brief "Cross-referencing every trait... Weighing shared
strengths..." beat before the result appears, on both the dedicated
Compare page and the inline widget on your result page.
Design note: this update's spec asked for a brighter multi-color
palette. Kept the noir black-and-white identity as primary since that
was an explicit earlier direction, and used the new accent colors only
on the MBTI/DISC/Enneagram/Big Five badges and duo crests, the genuinely
new content this update adds.
Tested the same way as every prior update: a real headless browser run
through onboarding, the full adaptive quiz, an edited answer, the PF1
upgrade flow, every new result section, and both compare entry points
with their new loading animation, zero console or page errors throughout.
